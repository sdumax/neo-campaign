...
function SheetOverlay({ className, ...props }: SheetPrimitive.Backdrop.Props) {
  return (
    <SheetPrimitive.Backdrop
      ...
      className={cn(
        "fixed inset-0 z-50 bg-black/70 transition-opacity duration-150 data-ending-style:opacity-0 data-starting-style:opacity-0 backdrop-blur-md",
        className
      )}
      {...props}
    />
        ...
        {showCloseButton && (
          <SheetPrimitive.Close
            ...
            render={
              <Button
                variant="ghost"
                className="absolute top-4 right-4 z-50 rounded-full text-zinc-400 hover:text-white hover:bg-zinc-800/80 size-8 p-0"
                size="icon-xs"
              />
            }
          >
